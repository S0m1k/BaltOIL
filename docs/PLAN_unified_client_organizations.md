...
- **Фаза 0** — ветка, этот план, аудит данных на проде (сколько company-профилей,
  заявок у них).
- **Фаза 1 (auth)** — таблицы Organization/Member, CRUD, членство, context с
  org_id; backfill-миграция company → org + owner. Деплой auth.
...
1. **Как сотрудник попадает в организацию?** Предлагаемый дефолт: owner добавляет
   по номеру телефона; если у номера ещё нет аккаунта — pending-приглашение,
   активируется при регистрации. Альтернатива: только admin привязывает.
2. **Права member vs owner**: member может создавать заявки от организации и видеть
   её документы; править реквизиты и состав — только owner/admin. Подтвердить.
3. **Тариф/кредит физлица**: оставляем на ClientProfile (как сейчас) — заявки «как
   физлицо» считаются по нему. Подтвердить, что это ожидаемо.
4. **Видимость заявок организации**: видит ли member все заявки организации или
   только свои? (влияет на фильтры в order_service).
```
