...
* Порт 8010 открыт в интернет и не аутентифицирован: подделать точку может любой, кто знает
  номер устройства. Защита — формат, частота, потолок автозаведения и возможность выдать
  трекеру токен (поле в БД готово). Для реальной защиты нужен модем с TLS/HMAC.
