...
1. `docker compose up -d --force-recreate delivery_service` — появились таблицы `gps_devices`
   и `gps_positions` (создаются автоматически при старте; отдельная миграция — `007_gps_tracking`).
2. `docker compose up -d --force-recreate frontend` — nginx начинает слушать **8010/tcp**
   (обычный HTTP, без TLS: GSM-модем трекера TLS не умеет).
3. Открыть порт на сервере: `ufw allow 8010/tcp` (или правило у хостера).
4. В трекере адрес приёма — `http://5.42.118.110:8010/`, путь любой.
5. Проверка без железа:
   `curl -s --data "SZTK-01 59.938732 30.312345 9" http://5.42.118.110:8010/` → `OK`.
   Пришедшее видно в админке: «Транспортные средства» → «Трекеры» → «Журнал приёма».

Что важно знать:

* Неизвестный номер устройства заводится сам (до 50 штук) и висит как «новый — подтвердите»;
  админ привязывает его к машине во вкладке «Трекеры». Отключается настройкой `GPS_AUTO_REGISTER=false`.
* История точек хранится 31 день, чистится фоновой задачей внутри `delivery_service` —
  cron на сервере не нужен.
* Приёмник однопроцессный: `delivery_service` запускается без `--workers`, антифлуд живёт
  в памяти процесса. Если когда-нибудь добавят воркеров — лимит станет на воркер.
* Приём доступен ТОЛЬКО через порт 8010: на 443 и 8003 путь `/gps/ingest` закрыт 404 (иначе
  ограничения приёмника — тело 8 КБ, узкий rate-limit — обходились бы через общий шлюз,
  где тело до 50 МБ).
* Порт 8010 открыт в интернет и не аутентифицирован: подделать точку может любой, кто знает
  номер устройства. **Риск принят осознанно** — прошивка трекеров токен не шлёт, а менять её
  нельзя. Что стоит сделать после первого запуска:
  1. убедиться, что все реальные трекеры заведены, и поставить `GPS_AUTO_REGISTER=false`
     в `delivery_service/.env` (по умолчанию `true`, чтобы не терять точки новых устройств);
  2. номера устройств делать неочевидными (не `SZTK-01`, а с случайным хвостом);
  3. когда появится модем с TLS/HMAC — выдать трекерам токены, поле в БД уже готово.
  Остаточная защита: формат, антифлуд по устройству, отдельный лимит на новые регистрации
  с одного адреса (не чаще раза в минуту), потолок автозаведения, лимит nginx 5 запросов/с
