...
## Ф12 — Звонки мобилки: гудки и полноэкранный входящий (2026-07-22, вечер)

**Репорт заказчика.** Исходящий — тишина до ответа; входящий при открытом
приложении — шторка-уведомление вместо экрана звонка, рингтон обрывается
после одного-двух гудков (MIUI глушит heads-up callkit).

**Решение.**
- **Гудки КПВ звонящему**: `assets/sounds/ringback.wav` (425 Гц, 1 с/4 с —
  российский стандарт, сгенерирован), играет в цикле через `audioplayers`,
  пока `remoteParticipants.isEmpty` («Ожидание ответа…»); стоп при входе
  собеседника/выходе. У принявшего входящий гудков нет (инициатор уже в комнате).
- **Полноэкранный входящий (форграунд)**: новый `IncomingCallScreen` —
  тёмный полный экран, имя звонящего, круглые «Принять»/«Отклонить»,
  системный рингтон в цикле (`flutter_ringtone_player`), poll статуса раз в
  2 с (инициатор отбил → экран сам закрывается). Показывается из пуша
  `onMessage` и из поллера-страховки (`_showIncoming` теперь пушит route
  вместо AlertDialog).
- **Фон/убитое приложение** — без изменений: системный callkit-экран
  (там рингтон не обрезается, экран поверх блокировки).

## Ф13 — Пересылка, стартовые чаты, надёжность звонка (2026-07-22, ночь)

- **Пересылка сообщений**: chat POST `/conversations/{target}/forward`
  {source_conversation_id, message_id} — проверка доступа к обоим диалогам,
  копия текста/типа; файл вложения физически копируется в каталог целевого
  диалога (пути conv-scoped); `metadata.forwarded_from` = имя автора.
  Мобилка: long-press → «Переслать» → список чатов; пометка «↪ Переслано от».
  Задеплоено (master 89ae2cd).
- **Стартовый экран сотрудника** — manager/admin попадают в чаты (как chats_only).
- **Звонок с блокировки «через раз»** — три причины и меры:
  1) фоновый изолят FCM: не было ensureInitialized → callkit молча падал
     на части запусков (исправлено + лог);
  2) ttl=45s на call-пуш: раньше пуш без ttl доставлялся ПОСЛЕ выхода из Doze,
     когда звонок давно истёк (фантомные/пропавшие показы);
  3) MIUI: если процесс убит свайпом/оптимизацией — FCM не может поднять
     приложение. Лечится ТОЛЬКО настройками телефона: Автозапуск = ВКЛ,
     Батарея = «Без ограничений» (Настройки → Приложения → СЗТК).

## Ф14 — Принятие звонка с заблокированного экрана (2026-07-22)

**Репорт.** Звонок не вылезает полным экраном на блокировке; после
разблокировки «Принять» не срабатывает.

**Причины и решение.**
1. «Принять» молчал: при холодном старте (приложение убито) событие accept
   приходило до готовности навигатора — `_onAccept` делал молчаливый return.
   Теперь ждём навигатор до 15 с (+ защита от двойного входа _joiningCallId).
2. Событие могло уйти совсем до подписки Dart: на старте HomeScreen
   `resumePendingAcceptedCall()` — смотрим `activeCalls()` с isAccepted
   и входим в комнату.
3. `showWhenLocked/turnScreenOn` на MainActivity: экран разговора открывается
   ПОВЕРХ блокировки — разблокировать не нужно.
4. Полноэкранный показ входящего на блокировке MIUI — нужно разрешение
   «Экран блокировки» (Настройки → Приложения → СЗТК → Другие разрешения →
   «Показывать на экране блокировки» = ВКЛ). Кодом не включается.

## Ф15 — Переносы строк в сообщениях + сессия мобилки (2026-07-24)

**Репорт заказчика.** (1) Многострочные сообщения (списки карт Киришей)
схлопываются в сплошной абзац — в вебе с телефона и в приложении.
(2) Приложение регулярно выкидывает из аккаунта.

**Переносы строк.** Причина — CSS: `.msg-text` без `white-space`, HTML
схлопывает `\n` в пробелы. Добавлен `white-space: pre-wrap` (перенос строк
сохраняется, длинные слова по-прежнему рвутся `word-break: break-word`).
Мобилка была в порядке: `TextField(maxLines: null)` + `Text()` переносы
сохраняют — правка только веб.

**Разлогин мобилки — корень: гонка refresh + reuse detection.**
Приложение шлёт запросы пачками (чаты, заявки, уведомления, WS). Когда
access-токен истекал, они получали 401 разом и КАЖДЫЙ дёргал
`/auth/refresh` со старым токеном. Сервер ротирует refresh при первом
обмене, а повторное предъявление того же токена трактует как кражу
(reuse detection) → `logout_all` сносит всю цепочку сессий → мгновенный
разлогин. Правки в `api_client.dart`:
- **single-flight refresh**: `_refreshInFlight` — параллельные 401 ждут один
  общий refresh вместо своего;
- **сессия не рвётся при сетевой ошибке**: чистим токены и уходим на логин
  только когда сервер явно ответил 401/403 на сам refresh (`_refreshRejected`);
  таймаут/обрыв связи больше не разлогинивает.
- `refresh_token_expire_days` 30 → **180** (токен ротируется при каждом
  обновлении, поэтому у активного пользователя сессия фактически бессрочна).
