...
Формат номера::
    ...
    ТТН-2026-Л000003   спец-ТТН (префикс зарезервирован, сейчас НЕ выдаётся)
...
* Ф начинает с 1 (у него собственный ключ, которого в БД ещё нет);
* Л заведён в реестре префиксов, но выдача запрещена (не входит в
  ``ISSUABLE_TTN_KINDS``) — критерий «специфической заявки» заказчицей пока
  не определён. Чтобы включить Л, достаточно добавить его в
  ``ISSUABLE_TTN_KINDS`` и описать правило в ``resolve_ttn_kind``.
...
class TtnKind(str, enum.Enum):
    ...
    SPECIAL = "special"        # Л — специфические заявки (пока не выдаётся)
...
ISSUABLE_TTN_KINDS: frozenset[TtnKind] = frozenset(
    {TtnKind.COMPANY, TtnKind.INDIVIDUAL}
)
...
#: Вид заявки → вид ТТН. ТТН-Л (внутренняя заявка менеджера) до появления
#: критерия Л остаётся на Ю-счётчике — том самом, которым она нумеровалась
#: и раньше, так что поведение для неё не меняется.
_ORDER_KIND_TO_TTN_KIND: dict[str, TtnKind] = {
    ...
    OrderKind.TTN_L.value: TtnKind.COMPANY,
}
